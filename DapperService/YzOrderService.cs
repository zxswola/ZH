...
{
    public class YzOrderService : IOrderService
    {
        ...
        public bool QueryOrderIsExit(string sourceOrderId)
        {
            ...
            {
                con.Open();
                string sql = "select t1.BILLID  from e_bl_order_hd t1 inner join E_INTERFACE_EXPRESS t2 on t1.BILLID=t2.BILLID where t1.sourceorderid =:sourceorderid ";
               return  con.Query(sql,new{ sourceorderid = sourceOrderId }).Any();
            }
        }

        public  bool UpdateOrderSourceTypeID(List<string> billIds)
        {
            using (var con = new OracleConnection(OracleHelper.connectionString))
            {
                con.Open();
                var transaction = con.BeginTransaction();
                string sql = " Update e_bl_order_hd set SourceTypeID='030' WHERE BILLID=:billid ";
                try
                {
                    foreach (var billid in billIds)
                    {
                         con.Execute(sql, new {billid = billid}, transaction);
                    }
